...
local Library do
    ...
    Library._DescendantsCache = {} -- Cache for GetDescendants results
    ...
    local function EnsureSharedRenderStepped()
        if not Library._SharedRenderStepped then
            Library._SharedRenderStepped = RunService.Heartbeat:Connect(UpdateOpenFrames)
        end
    end
        ...
        function Colorpicker:SetOpen(Bool)
            ...
            if self.IsOpen then 
                ...
                Library._FrameUpdaters[self] = function()
                    ...
                    local newX = button.AbsolutePosition.X
                    local newY = button.AbsolutePosition.Y + 26
                    -- Early exit if position hasn't changed
                    if lastPos and lastPos.X == newX and lastPos.Y == newY then
                        return
                    end
            ...
            -- Cache descendants for performance
            local colorpickerWindow = Items["ColorpickerWindow"].Instance
            if not Library._DescendantsCache[colorpickerWindow] then
                Library._DescendantsCache[colorpickerWindow] = colorpickerWindow:GetDescendants()
                TableInsert(Library._DescendantsCache[colorpickerWindow], colorpickerWindow)
            end
            local Descendants = Library._DescendantsCache[colorpickerWindow]
        ...
        function Keybind:SetOpen(Bool)
            ...
            if self.IsOpen then 
                ...
                Library._FrameUpdaters[self] = function()
                    ...
                    local newX = button.AbsolutePosition.X
                    local newY = button.AbsolutePosition.Y + 25
                    -- Early exit if position hasn't changed
                    if lastPos and lastPos.X == newX and lastPos.Y == newY then
                        return
                    end
            ...
            -- Cache descendants for performance
            local keybindWindow = Items["Window"].Instance
            if not Library._DescendantsCache[keybindWindow] then
                Library._DescendantsCache[keybindWindow] = keybindWindow:GetDescendants()
                TableInsert(Library._DescendantsCache[keybindWindow], keybindWindow)
            end
            local Descendants = Library._DescendantsCache[keybindWindow]
    ...
    do
        Library.Window = function(self, Data)
            ...
            function Window:SetOpen(Bool)
                ...
                if not Library._DescendantsCache[mainFrame] then
                    Library._DescendantsCache[mainFrame] = mainFrame:GetDescendants()
                    TableInsert(Library._DescendantsCache[mainFrame], mainFrame)
                end
                local Descendants = Library._DescendantsCache[mainFrame]
        ...
        Library.Sections.Dropdown = function(self, Data)
            ...
            function Dropdown:SetOpen(Bool)
                ...
                if not Library._DescendantsCache[optionHolder] then
                    Library._DescendantsCache[optionHolder] = optionHolder:GetDescendants()
                    TableInsert(Library._DescendantsCache[optionHolder], optionHolder)
                end
                local Descendants = Library._DescendantsCache[optionHolder]
